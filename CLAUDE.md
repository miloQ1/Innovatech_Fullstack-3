...
- `docker/mysql/init/02-seed-data.sql` is a full `mysqldump` (schema + rows) of demo data for `proyectos_db`, `recursos_db`, `colaboracion_db`, `analitica_db` (2 clients, 2 projects, phases/tasks, 3 professionals, a collaboration thread, and KPIs/widgets) — it includes its own `CREATE TABLE` statements so it doesn't need the microservices to have booted first. **Caveat**: like `01-init-databases.sql`, MySQL only runs `docker-entrypoint-initdb.d` scripts on a *fresh, empty* `mysql_data` volume. Anyone with a pre-existing volume (e.g. a teammate who already ran compose before this file existed) won't get it automatically — they need to either run `docker compose down -v` (wipes all local data, including `auth_db`/`notificaciones_db`) or import it by hand: `docker exec -i <mysql-container> mysql -uroot < docker/mysql/init/02-seed-data.sql`.
