...
const projectStatuses: ProjectStatus[] = ['PLANNING', 'IN_PROGRESS', 'ON_HOLD', 'COMPLETED', 'CANCELLED'];
...
export function ProjectDetailPage() {
  ...
  const [projectForm, setProjectForm] = useState({ code: '', name: '', description: '', status: 'PLANNING' as ProjectStatus, startDate: '', endDate: '', budget: '' });
    ...
    <div>
      ...
      <div className="page-header ion-margin-top">
        ...
        <div className="button-row">
          <IonBadge color={statusColor(project.status)}>{project.status.replace(/_/g, ' ')}</IonBadge>
              ...
              <IonList inset>
                ...
                <IonItem>
                  <IonSelect label="Estado" labelPlacement="stacked" value={projectForm.status} onIonChange={(e) => updateProjectForm('status', e.detail.value as ProjectStatus)}>
                    {projectStatuses.map((status) => <IonSelectOption key={status} value={status}>{status.replace(/_/g, ' ')}</IonSelectOption>)}
                  </IonSelect>
                </IonItem>
