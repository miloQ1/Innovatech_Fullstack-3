  ...
  return (
    <div>
      ...
      <div className="stat-grid">
        <IonCard className="app-card ion-text-center clickable" onClick={() => history.push('/projects')}>
          <IonCardContent><h2>{projects.length}</h2><p>Total proyectos</p></IonCardContent>
        </IonCard>
        <IonCard className="app-card ion-text-center clickable" onClick={() => history.push('/projects')}>
          <IonCardContent><h2>{inProgress}</h2><p>En progreso</p></IonCardContent>
        </IonCard>
        <IonCard className="app-card ion-text-center clickable" onClick={() => history.push('/clients')}>
          <IonCardContent><h2>{activeClients.length}</h2><p>Clientes activos</p></IonCardContent>
        </IonCard>
        <IonCard className="app-card ion-text-center clickable" onClick={() => history.push('/resources')}>
          <IonCardContent><h2>{professionalsCount}</h2><p>Profesionales</p></IonCardContent>
        </IonCard>
      </div>
      ...
      <div className="card-grid ion-margin-top">
        <IonCard className="app-card">
          <IonCardHeader><IonCardTitle>📁 Proyectos recientes</IonCardTitle></IonCardHeader>
          <IonCardContent>
            {latestProjects.length === 0 ? (
              ...
              <div key={getProjectId(project as Project & Record<string, unknown>) ?? project.code} className="list-row clickable" onClick={() => { const projectId = getProjectId(project as Project & Record<string, unknown>); if (projectId) history.push(`/projects/${projectId}`); }}>
                ...
                <IonBadge>{project.status.replace(/_/g, ' ')}</IonBadge>
              </div>
            ))}
          </IonCardContent>
        </IonCard>
        ...
        <IonCard className="app-card">
          <IonCardHeader><IonCardTitle>🏢 Clientes recientes</IonCardTitle></IonCardHeader>
          <IonCardContent>
